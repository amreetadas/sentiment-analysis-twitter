...
senti <- read.csv(paste0(path, "/Data/training.1600000.processed.noemoticon.csv"))
...
```{r}
model_name <- paste0(path,"LSTM",Sys.Date(),".h5")
save_model_hdf5(model, model_name)

tokenizer_config <- tokenizer$word_index
tokenizer_file <- paste0(path,"tokenizer",Sys.Date(),".json")
